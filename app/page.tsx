...
const MatrixBoxLoader = ({ message }: { message: string }) => (
    <div className="flex flex-col items-center justify-center p-10 border border-[#00ff41] bg-[#050505]/80">
        <div className="w-10 h-10 border-4 border-t-[#00ff41] border-r-transparent border-b-[#00ff41] border-l-transparent rounded-full animate-spin mb-4"></div>
        <p className="text-[#00ff41] font-mono text-sm animate-pulse tracking-widest">{message}</p>
    </div>
);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    getPostsByTag("other_certs").then(data => setDbOtherCerts(data as unknown as Post[]));
  ...
  return (
    ...
    <main>
        ...
        {isLoading ? (
            <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: '#050505', zIndex: 9999, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontFamily: "'VT323', monospace", padding: '20px' }}>
                ...
                <div style={{width: '100%', maxWidth: '600px', border: '1px solid #00ff41', padding: '20px', backgroundColor: 'rgba(0, 20, 0, 0.9)', boxShadow: '0 0 20px rgba(0, 255, 65, 0.2)'}}>
                    ...
                    <div ref={logsContainerRef} style={{height: '300px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '5px', color: '#00ff41', fontSize: '20px'}}>
                        {logs.map((log, index) => (<div key={index} style={{opacity: 0.8}}><span style={{color: '#008f11', marginRight: '10px'}}>[{new Date().toLocaleTimeString()}]</span>{`> ${log}`}</div>))}
                        {!showAccessGranted && <div style={{color: '#00ff41'}}>&gt; <span className="cursor-blink"></span></div>}
                    </div>
                    {showAccessGranted && <div style={{marginTop: '20px', borderTop: '1px dashed #00ff41', paddingTop: '15px', textAlign: 'center', fontSize: '40px', fontWeight: 'bold', color: '#00ff41', textShadow: '0 0 10px #00ff41'}} className="glitch-text">ACCESS GRANTED</div>}
                </div>
            </div>
        ) : (
            <>
                <section id="home" className="hero">
                    <div className="hero-text">
                        ...
                        {globalConfig?.isOpenForWork && <div className="mb-4 inline-flex items-center gap-2 border border-[#00ff41] px-3 py-1 rounded-full bg-[#00ff41]/10"><span className="w-2 h-2 rounded-full bg-[#00ff41] animate-pulse"></span><span className="text-xs text-[#00ff41] font-bold uppercase tracking-wider">{currentLang === 'vi' ? 'ĐANG TÌM VIỆC' : (currentLang === 'jp' ? '求職中' : 'OPEN FOR WORK')}</span></div>}
                        ...
                        <div className="btn-group">
                            ...
                            <Link href="/faq" className="btn text-[#00ff41] border-[#00ff41] hover:bg-[#00ff41] hover:text-black">FAQ_HELP</Link>
                        </div>
                    </div>
                    <div className="hero-img-large"><Image src={hero.avatarUrl || "/pictures/VuTriDung.jpg"} alt={hero.fullName} width={350} height={350} className="object-cover border-4 border-[#00ff41] shadow-[0_0_20px_#00ff41]" /></div>
                </section>

                <section id="about" className="content-section"><h2>{t.sec_about}</h2>{aboutText ? <p style={{whiteSpace: 'pre-line'}}>{aboutText}</p> : <NoDataDisplay section="ABOUT ME" />}</section>
                <section id="profile" className="content-section"><h2>{t.sec_profile}</h2>{profileBoxes && profileBoxes.length > 0 ? (<div className="profile-container">{profileBoxes.map((box) => (<div key={box.id} className="profile-box"><h3>{box.title}</h3><ul className="profile-list">{box.items.map((item, idx) => (<li key={idx}><span className="label">{item.label}</span><span className="value">{item.value}</span></li>))}</ul></div>))}</div>) : <NoDataDisplay section="PROFILE" />}</section>
                <section id="certificates" className="content-section"><h2>{t.sec_cert}</h2><h3 className="carousel-title">{t.cat_lang}</h3><div className="carousel-wrapper"><button className="nav-btn prev-btn" onClick={() => scrollCarousel('lang-certs', -1)}>&#10094;</button><div className="carousel-container" id="lang-certs">{dbLangCerts.length > 0 ? (dbLangCerts.map((post) => (<Link key={post.id} href={`/blog/${post.id}`} className="card block text-decoration-none"><img src={getCoverImage(post.images)} alt={post.title} style={{height: 160, width: '100%', objectFit: 'cover'}} /><div className="card-info"><h4>{post.title}</h4><p className="text-[#00ff41] text-xs mt-1">&gt;&gt; VIEW CERT</p></div></Link>))) : (<div className="text-gray-500 italic p-5 border border-dashed border-[#333] w-full text-center">NO CERTIFICATES</div>)}</div><button className="nav-btn next-btn" onClick={() => scrollCarousel('lang-certs', 1)}>&#10095;</button></div>
                
                <h3 className="carousel-title" style={{marginTop: 40}}>{t.cat_tech}</h3><div className="carousel-wrapper"><button className="nav-btn prev-btn" onClick={() => scrollCarousel('tech-certs', -1)}>&#10094;</button><div className="carousel-container" id="tech-certs">{dbTechCerts.length > 0 ? (dbTechCerts.map((post) => (<Link key={post.id} href={`/blog/${post.id}`} className="card block text-decoration-none"><img src={getCoverImage(post.images)} alt={post.title} style={{height: 160, width: '100%', objectFit: 'cover'}} /><div className="card-info"><h4>{post.title}</h4><p className="text-[#00ff41] text-xs mt-1">&gt;&gt; VIEW CERT</p></div></Link>))) : (<div className="text-gray-500 italic p-5 border border-dashed border-[#333] w-full text-center">NO CERTIFICATES</div>)}</div><button className="nav-btn next-btn" onClick={() => scrollCarousel('tech-certs', 1)}>&#10095;</button></div>

                <h3 className="carousel-title" style={{marginTop: 40}}>{currentLang === 'vi' ? 'CHỨNG CHỈ KHÁC' : (currentLang === 'jp' ? 'その他の証明書' : 'OTHER CERTS')}</h3><div className="carousel-wrapper"><button className="nav-btn prev-btn" onClick={() => scrollCarousel('other-certs', -1)}>&#10094;</button><div className="carousel-container" id="other-certs">{dbOtherCerts.length > 0 ? (dbOtherCerts.map((post) => (<Link key={post.id} href={`/blog/${post.id}`} className="card block text-decoration-none"><img src={getCoverImage(post.images)} alt={post.title} style={{height: 160, width: '100%', objectFit: 'cover'}} /><div className="card-info"><h4>{post.title}</h4><p className="text-[#00ff41] text-xs mt-1">&gt;&gt; VIEW CERT</p></div></Link>))) : (<div className="text-gray-500 italic p-5 border border-dashed border-[#333] w-full text-center">NO CERTIFICATES</div>)}</div><button className="nav-btn next-btn" onClick={() => scrollCarousel('other-certs', 1)}>&#10095;</button></div></section>
                <section id="career" className="content-section"><h2>{t.sec_career}</h2>{careerText ? <p style={{whiteSpace: 'pre-line'}}>{careerText}</p> : <NoDataDisplay section="CAREER GOALS" />}</section>
                ...
                <section id="skills" className="content-section"><h2>{t.sec_skills}</h2>{skillsText ? <p style={{whiteSpace: 'pre-line'}}>{skillsText}</p> : <NoDataDisplay section="SKILLS" />}</section>
                <section id="experience" className="content-section"><h2>{t.sec_exp}</h2>{experienceBoxes && experienceBoxes.length > 0 ? (<div className="profile-container">{experienceBoxes.map((box) => (<div key={box.id} className="profile-box"><h3>{box.title}</h3><ul className="profile-list">{box.items.map((item, idx) => (<li key={idx}><span className="label">{item.label}</span><span className="value">{item.value}</span></li>))}</ul></div>))}</div>) : <NoDataDisplay section="EXPERIENCE" />}</section>
                <section id="projects" className="content-section">
                    ...
                    <div className="mb-8 border border-[#00ff41] p-4 bg-[#050505] flex flex-wrap gap-4 justify-between items-center shadow-[0_0_10px_rgba(0,255,65,0.2)]">
                        <div className="flex gap-4 items-center flex-wrap"><span className="text-[#00ff41] font-bold text-sm tracking-widest">&gt; FILTER_LANG:</span>{[{v: "ALL", l: "ALL"}, {v: "vi", l: "VI"}, {v: "en", l: "EN"}, {v: "jp", l: "JP"}].map(opt => (<button key={opt.v} onClick={() => setProjLang(opt.v)} className={`px-3 py-1 font-mono text-sm border border-[#00ff41] transition-all hover:shadow-[0_0_5px_#00ff41] ${projLang === opt.v ? 'bg-[#00ff41] text-black font-bold' : 'bg-transparent text-[#00ff41]'}`}>[{opt.l}]</button>))}</div>
                        <button onClick={() => setProjSort(prev => prev === "newest" ? "oldest" : "newest")} className="flex items-center gap-2 text-[#00ff41] hover:text-white transition-colors text-sm font-mono border-b border-transparent hover:border-[#00ff41]"><span>{projSort === "newest" ? "▼" : "▲"}</span><span>SORT: {projSort === "newest" ? "LATEST" : "OLDEST"}</span></button>
                    </div>
                    {[ { title: t.cat_uni_proj, data: dbUniProjects, id: 'uni-projects' }, { title: t.cat_personal_proj, data: dbPersonalProjects, id: 'personal-projects' } ].map((cat, idx) => {
                        ...
                        return (<div key={idx}><h3 className="carousel-title">{cat.title} <span className="text-sm opacity-60">({filtered.length})</span></h3><div className="carousel-wrapper"><button className="nav-btn prev-btn" onClick={() => scrollCarousel(cat.id, -1)} >&#10094;</button><div className="carousel-container" id={cat.id}>{filtered.length > 0 ? (filtered.map((post) => (
                            <Link key={post.id} href={`/blog/${post.id}`} className="card block text-decoration-none">
                                {/* [FIXED] h-[160px] -> h-40 */}
                                <div className="relative h-40 w-full overflow-hidden border-b border-[#00ff41]">
                            ...
                            </Link>))) : (<div className="text-gray-500 italic p-5 border border-dashed border-[#333] w-full text-center">NO DATA MATCHING FILTER [{projLang}]</div>)}</div><button className="nav-btn next-btn" onClick={() => scrollCarousel(cat.id, 1)} >&#10095;</button></div></div>);
                    })}
                </section>
                ...
                <section id="contact" className="content-section" style={{marginBottom: 100}}>
                    <h2>11. CONTACT</h2>
                    <div className="text-center mb-10"><p className="font-mono text-[#00ff41] text-sm md:text-base animate-pulse">{currentLang === 'vi' ? '>> THIẾT LẬP KẾT NỐI AN TOÀN...' : (currentLang === 'jp' ? '>> 安全な接続を確立中...' : '>> ESTABLISHING_SECURE_CONNECTION...')}</p><p className="text-gray-500 text-xs mt-2">{currentLang === 'vi' ? '[ Hãy cùng tạo ra những điều tuyệt vời! ]' : (currentLang === 'jp' ? '[ 一緒に素晴らしいものを作りましょう！ ]' : '[ Let\'s create something beautiful together! ]')}</p></div>
                    {isLoading ? <MatrixBoxLoader message="DECRYPTING_CONTACT_PROTOCOLS..." /> : (contactBoxes && contactBoxes.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto">{contactBoxes.map((box) => (<div key={box.id} className="border border-[#00ff41] bg-[#050505] p-10 relative group hover:shadow-[0_0_20px_rgba(0,255,65,0.3)] transition-all duration-300"><div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-[#00ff41]"></div><div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-[#00ff41]"></div><h3 className="text-2xl font-bold text-[#00ff41] border-b border-[#00ff41] pb-4 mb-6 uppercase tracking-widest flex items-center gap-3"><span className="text-sm opacity-50">0x{box.id.substring(0,2)}</span> {box.title}</h3><div className="flex flex-col gap-6 font-mono text-lg">{box.items.map((item, idx) => { let content; const val = item.value; if (val.includes('@')) { content = (<a href={`mailto:${val}`} className="text-[#00ff41] hover:bg-[#00ff41] hover:text-black transition-colors px-2 py-1 inline-block break-all"><span className="opacity-50 mr-2">[MAIL]:</span>{val}</a>); } else if (val.startsWith('http')) { content = (<a href={val} target="_blank" rel="noopener noreferrer" className="text-[#00ff41] hover:underline decoration-dashed break-all px-2 py-1 inline-block"><span className="opacity-50 mr-2">[LINK]:</span>{val} ↗</a>); } else if (val.match(/^[0-9+ ]+$/) && val.length > 8) { content = (<a href={`tel:${val.replace(/\s/g, '')}`} className="text-[#00ff41] font-bold hover:bg-[#00ff41] hover:text-black px-2 py-1 inline-block transition-colors"><span className="opacity-50 mr-2">[CALL]:</span>{val}</a>); } else { content = <span className="text-gray-300 px-2 py-1"><span className="text-[#00ff41] opacity-50 mr-2">&gt;</span>{val}</span>; } 
                        return (<div key={idx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-[#00ff41]/20 pb-4 last:border-0">
                            {/* [FIXED] min-w-[120px] -> min-w-30 (Theo suggest) */}
                            <span className="text-gray-500 font-bold text-sm uppercase mb-2 sm:mb-0 min-w-30">{item.label}_</span>
                            <div className="text-right">{content}</div></div>); })}</div></div>))}</div>) : <NoDataDisplay section="CONTACT" />)}
                </section>
            </>
        )}
        
        {/* ĐƯA AI CHATBOX RA NGOÀI ĐỂ TRÁNH LỖI VỊ TRÍ */}
